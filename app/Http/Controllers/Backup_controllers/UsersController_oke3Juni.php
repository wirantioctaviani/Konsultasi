...
class UsersController extends Controller
{
	    	...
	    	if ($subbid_id == "11")
	    	{
							$check = DB::select(DB::raw('select a.active_pic from
										(select DISTINCT
										        ...
										        where mstr_role_id = "4" and a.subbid_id = "11" 
										 ) a 
							             WHERE active_pic = "no"'));
							#dd("11");
							...
							if ($check == null){
											...
											$check_pic = DB::select(DB::raw('select id from(
												...
												(select 
												        ...
												        where mstr_role_id = "4" and a.subbid_id = "11") x
												      -- Cari case terbanyak
												        left join 
												        (
									    ...
									    $request->file('dokumen')->move('dokumen/', $nama_file);}
									        ...
									        return redirect('/konsul_online')->with('sukses', 'Form Berhasil di Submit!'); 
			}
			...
			elseif ($subbid_id == "12")
	    	{
							...
							$check = DB::select(DB::raw('select a.active_pic from
										(select DISTINCT
										        ...
										        where mstr_role_id = "4" and a.subbid_id = "12" 
										 ) a 
							             WHERE active_pic = "no"'));
							#dd("11");
	...
	public function respon_konsulonline($id_konsul)
    {
    	...
    	return view('users.respon2_konsulonline')->with(['data_konsul_online' => $data_konsul_online])->with(['data_konsul_detail' => $data_konsul_detail]);
    }
  ...
  //               	['updated_date' => date('Y-m-d H:i:s')]);
		...
		$data_detail->created_by 	= 'wiranti';
