...
use App\Model\tb_faq_subkategori;
...
class UsersController extends Controller
{
	...
	public function konsul_online()
	    {
            ...
            ->selectraw('layanan_id,jenis_layanan, count(tb_konsul_online.id_konsul) as jumlah')
            ->groupby('jenis_layanan','layanan_id')
            ->orderby('layanan_id')
            ->get();
            // dd($data_konsul_online);

			$open = DB::select(DB::raw('SELECT
						a.id as layanan_id,
						count( b.id_konsul ) as jumlahopen
					FROM
						mstr_layanan a
						LEFT JOIN tb_konsul_online b ON b.layanan_id = a.id 
						AND ( b.status_id = 1 ) 
					GROUP BY
						a.id
					ORDER BY a.id'));
            ...
            $proses = DB::select(DB::raw('SELECT
						a.id as layanan_id,
						count( b.id_konsul ) as jumlahproses
					FROM
						mstr_layanan a
						LEFT JOIN tb_konsul_online b ON b.layanan_id = a.id 
						AND ( b.status_id = 0 or b.status_id = 2 or b.status_id = 3 ) 
					GROUP BY
						a.id
					ORDER BY a.id'));
            // dd($proses);

            $closed = DB::select(DB::raw('SELECT
						a.id as layanan_id,
						count( b.id_konsul ) as jumlahclosed
					FROM
						mstr_layanan a
						LEFT JOIN tb_konsul_online b ON b.layanan_id = a.id 
						AND ( b.status_id = 4 ) 
	    	...
	    	->orderby('subbid_id', 'ASC')
	    	->get();
	    	return view('homepage.homepage',['data_layanan' => $data_layanan, 'data_konsul_online' => $data_konsul_online, 'open' => $open, 'proses' => $proses, 'closed' => $closed]);
	    }
	...
	public function formkonsul()
	    {
           ...
           // 	$open = \App\Models\tb_konsul_online::leftjoin('mstr_layanan', function($join) {
          	// $join->on('tb_konsul_online.layanan_id', '=', 'mstr_layanan.id');})
           //  ->selectraw('layanan_id, count(tb_konsul_online.id_konsul) as jumlahopen')
           //  ->where('status_id','=',1)
           //  ->groupby('layanan_id')
           //  ->orderby('layanan_id')
           //  ->get();
           ...
           //  $proses = \App\Models\tb_konsul_online::join('mstr_layanan', function($join) {
          	// $join->on('tb_konsul_online.layanan_id', '=', 'mstr_layanan.id');})
           //  ->selectraw('layanan_id, count(tb_konsul_online.id_konsul) as jumlahproses')
           //  ->where('status_id','=',0)
           //  ->orwhere('status_id','=',2)
           //  ->orwhere('status_id','=',3)
           //  ->groupby('layanan_id')
           ...
           //  $closed = \App\Models\tb_konsul_online::leftjoin('mstr_layanan', function($join) {
          	// $join->on('tb_konsul_online.layanan_id', '=', 'mstr_layanan.id');})
           //  ->selectraw('layanan_id, count(tb_konsul_online.id_konsul) as jumlahclosed')
           //  ->where('status_id',4)
           //  ->groupby('layanan_id')
           //  ->orderby('layanan_id')
           //  ->get();
	...
	public function hasilpencarian(Request $request)
	    {
	    	// dd($request->all());
	    	$count = DB::table('faq_subkategori')
	    	->join('faq_kategori', 'faq_subkategori.kategori_id', '=', 'faq_kategori.id_kategori')
	    	->select(DB::raw('COUNT(id_subkategori) as jumlahpencarian'))
            ->where('jawaban', 'like', '%' . $request->keyword . '%')
    		->orWhere('nama_subkategori', 'like', '%' . $request->keyword . '%')
            ->first();
            // dd($count);

	    	$cari = DB::table('faq_subkategori')
	    	->join('faq_kategori', 'faq_subkategori.kategori_id', '=', 'faq_kategori.id_kategori')
	    	->join('mstr_layanan', 'faq_kategori.layanan_id', '=', 'mstr_layanan.id')
	    	->select('id_subkategori', 'nama_subkategori', 'jawaban', 'id_kategori', 'nama_kategori', 'layanan_id', 'jenis_layanan')
            ->where('jawaban', 'like', '%' . $request->keyword . '%')
    		->orWhere('nama_subkategori', 'like', '%' . $request->keyword . '%')
            ->get();
            // dd($cari);
	    	return view('homepage.hasilpencarian',['cari' => $cari, 'count' => $count]);
	    }
	...
	public function detailfaq(Request $request)
	    {	
	    	...
	    	// dd($id_subkategori);
	    	// dd([$id_kategori, $id_subkategori]);
	    	$kategori = DB::table('faq_kategori')
	    	->where('layanan_id', $request->layanan_id)
            ->get();
	    	// dd($kategori);
	    	
            $subkategori = DB::table('faq_subkategori')
            ->leftjoin('faq_kategori', 'faq_subkategori.kategori_id', '=', 'faq_kategori.id_kategori')
	    	->leftjoin('mstr_layanan', 'faq_kategori.layanan_id', '=', 'mstr_layanan.id')
	    	->select('id_subkategori', 'nama_subkategori', 'jawaban', 'kategori_id', 'nama_kategori', 'layanan_id', 'jenis_layanan')
            ->get();
            // dd($subkategori);

	    	$subkategorishow = DB::table('faq_subkategori')
	    	->leftjoin('faq_kategori', 'faq_subkategori.kategori_id', '=', 'faq_kategori.id_kategori')
	    	->leftjoin('mstr_layanan', 'faq_kategori.layanan_id', '=', 'mstr_layanan.id')
	    	->select('id_subkategori', 'nama_subkategori', 'jawaban', 'id_kategori', 'nama_kategori', 'layanan_id', 'jenis_layanan')
	    	...
	    	return view('homepage.detailfaq',['kategori' => $kategori,'subkategori' => $subkategori,'subkategorishow' => $subkategorishow,'id_subkategori' => $id_subkategori,'id_kategori' => $id_kategori]);
	    }  


	// public function faqdetail($id)
	//     {
	//     	// return DB::table('faq_subkategori')->where('id_subkategori', $id)->get();
	//     	$data = \App\Models\tb_faq_subkategori::where('id_subkategori',$id)->first();
    ...
    public function create_konsul(Request $request)
	    {
	    	...
	    	$user_subkoor = DB::table('users')->where('mstr_role_id', '3')->where('is_active', '1')->where('subbid_id', $subbid_id)->value('id');
	    	$email_subkoor = DB::table('users')->where('mstr_role_id', '3')->where('is_active', '1')->where('subbid_id', $subbid_id)->value('email');
	    	...
	    	$rules = array(
		        'dokumen' => 'mimetypes:application/pdf'
		    );
		    ...
		    if ( $validator->fails() ) {
		        ...
		        return redirect()->action('App\Http\Controllers\UsersController@formkonsul')->with('gagalupload', 'File yang diupload harus dalam format PDF');
		    }
		    else{
			    if ($subbid_id == "11")
		    	{
		    		// dd('11');
								...
								if ($check == null){
										    ...
										    $request->file('dokumen')->move(public_path('dokumen/'), $nama_file);}
										        ...
										        return redirect()->action('App\Http\Controllers\UsersController@formkonsul')->with('sukses', 'Terima kasih, pertanyaan anda telah terkirim. Mohon periksa email Anda secara berkala untuk memantau progres jawaban dari Pusbin JFA.');
				}
	...
	public function respon_konsulonline($id_konsul)
    {
	    ...
	    $rating = \App\Models\rating::where('id_konsul', $id_konsul) 
	    ->get();
	    // dd($rating);

    	// dd($data_konsul_detail); 
    	return view('users.detailkonsultasi')->with(['data_konsul_online' => $data_konsul_online])->with(['data_konsul_detail' => $data_konsul_detail])->with(['rating' => $rating]);
    }
    ...
    public function reply_konsulonline($id_konsul, Request $request)
    {
    	...
    	$rules = array(
		    ...
		    if ( $validator->fails() ) {
		        # if validations fails redirect back with errors
		        return redirect()->action('App\Http\Controllers\UsersController@respon_konsulonline', [encrypt($id_konsul)])->with('gagalupload', 'File yang diupload harus dalam format PDF');
		    }
		    else
		    {
