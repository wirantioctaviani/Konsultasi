...
class UsersController extends Controller
{
	...
	public function konsul_online()
	    {
	    	...
	    	->orderby('subbid_id', 'ASC')
	    	->get();
	    	return view('homepage',['data_layanan' => $data_layanan, 'data_konsul_online' => $data_konsul_online, 'open' => $open, 'closed' => $closed]);
	    }
	    		...
	    		// dd('11');
							...
							if ($check == null){
									    ...
									    $request->file('dokumen')->move(public_path('dokumen/'), $nama_file);}
									        ...
									        return redirect()->action('App\Http\Controllers\UsersController@konsul_online')->with('sukses', 'Terima kasih, pertanyaan anda telah terkirim. Mohon periksa email Anda secara berkala untuk memantau progres jawaban dari Pusbin JFA.');
			}
	...
	public function respon_konsulonline($id_konsul)
    {
    	...
    	return view('users.DetailKonsulOnline')->with(['data_konsul_online' => $data_konsul_online])->with(['data_konsul_detail' => $data_konsul_detail])->with(['rating' => $rating]);
    }
